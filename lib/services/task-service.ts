import { Task, CreateTaskRequest, UpdateTaskRequest, SearchFilters, PaginationParams } from '@/lib/types';
...
class TaskService {
  ...
  async bulkUpdateTasks(ids: string[], data: UpdateTaskRequest): Promise<void> {
    try {
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError('Failed to bulk update tasks', 'BULK_UPDATE_ERROR', error);
    }
  }
  ...
  async bulkDeleteTasks(ids: string[]): Promise<void> {
    try {
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError('Failed to bulk delete tasks', 'BULK_DELETE_ERROR', error);
    }
  }
}
