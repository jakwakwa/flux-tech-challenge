...
import { Task, SearchFilters, UpdateTaskRequest } from '@/lib/types';
...
interface TaskState {
  ...
  createTask: (data: { title: string; description?: string; listId: string }) => Promise<Task | null>;
    ...
    subscribeWithSelector(
      immer((set, get) => ({
        ...
        deleteCompletedTasks: async (listId) => {
          const completedTasks = get().tasks.filter(t => 
            t.completed && (!listId || t.listId === listId)
          );
          ...
          try {
            ...
            set((state) => {
              ...
              state.error = error instanceof Error ? error.message : 'Failed to delete completed tasks';
            });
          }
        },
        ...
        markAllAsComplete: async (listId) => {
          const pendingTasks = get().tasks.filter(t => 
            !t.completed && (!listId || t.listId === listId)
          );
          ...
          try {
            ...
            set((state) => {
              // Restore on error
              state.tasks = originalTasks;
              state.error = error instanceof Error ? error.message : 'Failed to mark tasks as complete';
            });
          }
        },
        ...
        getTasksByListId: (listId) => get().tasks.filter(t => t.listId === listId),
