...
import type { ApiResponse, SearchParams, Task } from "@/lib/types";
import type {
	ApiErrorResponse,
	ApiSuccessResponse,
...
export function useTasks(options: UseTasksOptions = {}): UseTasksReturn {
	...
	const fetchTasks = useCallback(async () => {
		...
		// biome-ignore lint/correctness/useExhaustiveDependencies: < Search params are not stable>
	...
	const createTask = useCallback(
		async (data: CreateTaskInput) => {
			try {
				...
				if (!response.ok) {
					const errorData = result as unknown as ApiErrorResponse;
					throw new Error(errorData?.error?.message || "Failed to create task");
				}

				const successData = result as ApiSuccessResponse;
				if (!successData.success) {
