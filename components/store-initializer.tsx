...
export function StoreInitializer({ lists, tasks }: StoreInitializerProps) {
  const { lists: currentLists, fetchLists } = useListStore();
  const { tasks: currentTasks, fetchTasks } = useTaskStore();

  useEffect(() => {
    // Only initialize lists if store is empty and we have server data
    if (lists && lists.length > 0 && currentLists.length === 0) {
