...
export function ListGrid() {
  ...
  const handleDeleteList = async (id: string, title: string) => {
    if (!confirm(`Are you sure you want to delete "${title}"? This will also delete all tasks in this list.`)) {
      return;
    }
      ...
      {lists.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {lists.map((list) => {
            ...
            return (
              <Card key={list.id} className="relative group hover:shadow-md transition-shadow">
                <CardHeader className="flex flex-row items-start justify-between space-y-0">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => router.push(`/dashboard?listId=${list.id}`)}>
                        <ArrowRight className="h-4 w-4 mr-2" />
                        View Tasks
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => openModal('editList', { listId: list.id })}>
                ...
                <CardContent>
                  ...
                  <div className="flex justify-end gap-2 mt-4">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => router.push(`/dashboard?listId=${list.id}`)}
                    >
                      View
                    </Button>
                    <Button
