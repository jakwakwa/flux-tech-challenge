...
export function DashboardTableClient({
	...
	const { lists, deleteList, isDeleting, fetchLists, hasBeenModified } =
		useListStore();
	...
	const handleDeleteList = async (id: string, title: string) => {
		try {
			...
			// Refresh data to keep client and server in sync
			router.refresh();
		} catch (error) {
			addToast({
				type: "error",
												...
												<AlertDialogContent>
													<AlertDialogHeader>
														...
														<AlertDialogDescription>
															...
															permanently delete the list "{list.title}" and all{" "}
